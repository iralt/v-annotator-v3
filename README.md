...
### 1. ライブラリエクスポートの不整合
- `src/index.ts` が古い `DialogTest` コンポーネントをエクスポートしている
- 実際のメインコンポーネント `VAnnotator` がエクスポートされていない
- プラグイン登録で正しいコンポーネントが登録されていない

### 2. TypeScriptタイプエクスポート
- 公開APIの型定義（`EntityData`, `CandidateEntity` など）が外部から利用できない
- ライブラリ利用者が型安全性を享受できない状態

### 3. パフォーマンス関連
- `vue-virtual-scroller` の統合が不完全（コメントアウト状態）
